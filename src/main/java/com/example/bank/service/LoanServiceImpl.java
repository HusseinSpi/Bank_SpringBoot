...
public class LoanServiceImpl implements LoanService {
    ...
    @Autowired
    public LoanServiceImpl(LoanRepository loanRepository, AccountRepository accountRepository, ForexService forexService) {
...
public Loan payLoan(Long loanId, BigDecimal paymentAmount) {
    ...
    // Get the customer's account and convert the payment amount if needed.
    Account account = loan.getAccount();
    // Check if the payment currency (loan.getCurrency()) differs from the account's currency.
    BigDecimal convertedPayment = paymentAmount;
    if (!loan.getCurrency().equalsIgnoreCase(account.getCurrency().name())) {
        convertedPayment = forexService.convert(paymentAmount, loan.getCurrency(), account.getCurrency().name());
    }

    // Withdraw the (converted) payment amount from the customer's account.
    account.withdraw(convertedPayment, null);
